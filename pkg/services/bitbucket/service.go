...
type Service interface {
	...
	PublishBitbucketEvent(ctx context.Context, event manifest.ZiplineeBitbucketEvent) (err error)
...
func NewService(config *api.APIConfig, bitbucketapiClient bitbucketapi.Client, pubsubapiClient pubsubapi.Client, ziplineeService ziplinee.Service, queueService queue.Service) Service {
	return &service{
		config:             config,
...
func (s *service) CreateJobForBitbucketPush(ctx context.Context, installation bitbucketapi.BitbucketAppInstallation, pushEvent bitbucketapi.RepositoryPushEvent) (err error) {
	...
	if err != nil {
		log.Error().Err(err).
			Msg("Retrieving Ziplinee manifest failed")
		return err
	}

	// get manifest file
	manifestExists, manifestString, err := s.bitbucketapiClient.GetZiplineeManifest(ctx, accessToken, pushEvent)
	if err != nil {
		log.Error().Err(err).
			Msg("Retrieving Ziplinee manifest failed")
		return err
...
func (s *service) PublishBitbucketEvent(ctx context.Context, event manifest.ZiplineeBitbucketEvent) (err error) {
...
func (s *service) Rename(ctx context.Context, fromRepoSource, fromRepoOwner, fromRepoName, toRepoSource, toRepoOwner, toRepoName string) (err error) {
	return s.ziplineeService.Rename(ctx, fromRepoSource, fromRepoOwner, fromRepoName, toRepoSource, toRepoOwner, toRepoName)
}

func (s *service) Archive(ctx context.Context, repoSource, repoOwner, repoName string) (err error) {
	return s.ziplineeService.Archive(ctx, repoSource, repoOwner, repoName)
}

func (s *service) Unarchive(ctx context.Context, repoSource, repoOwner, repoName string) (err error) {
	return s.ziplineeService.Unarchive(ctx, repoSource, repoOwner, repoName)
}
