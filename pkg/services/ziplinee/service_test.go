...
func TestRename(t *testing.T) {
	...
	t.Run("CallsRenameOndatabaseClient", func(t *testing.T) {
		...
		// act
		err := service.Rename(ctx, "github.com", "ziplineeci", "ziplinee-ci-contracts", "github.com", "ziplineeci", "ziplinee-ci-protos")
...
func TestUpdateBuildStatus(t *testing.T) {
	t.Run("CallsUpdateBuildStatusOndatabaseClientIfBuildIDIsNonEmpty", func(t *testing.T) {
		...
		event := contracts.ZiplineeCiBuilderEvent{
...
func TestUpdateJobResources(t *testing.T) {
	t.Run("CallsUpdateBuildResourceUtilizationOndatabaseClientIfBuildIDIsNonEmpty", func(t *testing.T) {
		...
		event := contracts.ZiplineeCiBuilderEvent{
			PodName: "build-ziplinee-ziplinee-ci-api-123456-mhrzk",
			JobType: contracts.JobTypeBuild,
			Build: &contracts.Build{
				ID:          "123456",
				BuildStatus: contracts.StatusSucceeded,
	...
	t.Run("CallsUpdateReleaseResourceUtilizationOndatabaseClientIfReleaseIDIsNonEmpty", func(t *testing.T) {
		...
		event := contracts.ZiplineeCiBuilderEvent{
			PodName: "release-ziplinee-ziplinee-ci-api-123456-mhrzk",
			JobType: contracts.JobTypeRelease,
			Release: &contracts.Release{
				ID:            "123456",
				ReleaseStatus: contracts.StatusSucceeded,
...
func TestGetEventsForJobEnvvars(t *testing.T) {
	t.Run("ReturnsTriggerEventsForEveryNamedTriggerWithAtLeastOneGreenBuild", func(t *testing.T) {
		...
		databaseClient.EXPECT().GetPipelineBuilds(gomock.Any(), gomock.Eq("github.com"), gomock.Eq("ziplineeci"), gomock.Eq("repo1"), gomock.Eq(1), gomock.Eq(1), gomock.Any(), gomock.Any(), gomock.Eq(false)).
		...
		databaseClient.EXPECT().GetPipelineBuilds(gomock.Any(), gomock.Eq("github.com"), gomock.Eq("ziplineeci"), gomock.Eq("repo2"), gomock.Eq(1), gomock.Eq(1), gomock.Any(), gomock.Any(), gomock.Eq(false)).
			DoAndReturn(func(ctx context.Context, repoSource, repoOwner, repoName string, pageNumber, pageSize int, filters map[api.FilterType][]string, sortings []api.OrderField, optimized bool) (builds []*contracts.Build, err error) {
				return []*contracts.Build{
					{
						BuildStatus:  contracts.StatusSucceeded,
		...
		triggers := []manifest.ZiplineeTrigger{
			{
				Name: "trigger1",
				Pipeline: &manifest.ZiplineePipelineTrigger{
					Name:   "github.com/ziplineeci/repo1",
					Branch: "main",
					Event:  "finished",
					Status: "succeeded",
				},
				BuildAction: &manifest.ZiplineeTriggerBuildAction{
					Branch: "main",
				},
			},
			{
				Name: "trigger2",
				Pipeline: &manifest.ZiplineePipelineTrigger{
					Name:   "github.com/ziplineeci/repo2",
					Branch: "main",
					Event:  "finished",
					Status: "succeeded",
				},
