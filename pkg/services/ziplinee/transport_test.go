...
func TestGetPipeline(t *testing.T) {
	...
	t.Run("ReturnsPipelineFromNewdatabaseClientAfterReload", func(t *testing.T) {
		...
		c.Request = httptest.NewRequest("GET", "https://ci.ziplinee.io/pipelines/a/b/c", bodyReader)
...
func TestGetManifestTemplates(t *testing.T) {
	t.Run("ReturnsTemplatesCorrectly", func(t *testing.T) {
		...
		c, _ := gin.CreateTestContext(recorder)
		bodyReader := strings.NewReader("")
		c.Request = httptest.NewRequest("GET", "https://ci.ziplinee.io/manifest/templates", bodyReader)
		if !assert.NotNil(t, c.Request) {
			return
		}
...
func TestGenerateManifest(t *testing.T) {
	t.Run("ReturnsManifestFromTemplate", func(t *testing.T) {
		...
		c.Request = httptest.NewRequest("POST", "https://ci.ziplinee.io/manifest/generate", bodyReader)
		if !assert.NotNil(t, c.Request) {
			return
		}
...
func TestCreatePipelineRelease_Forbidden(t *testing.T) {
	t.Run("ReturnsForbiddenError", func(t *testing.T) {
				...
				builds = []*contracts.Build{
					{
						...
						ReleaseTargets: []contracts.ReleaseTarget{
							{
								...
								Actions: []manifest.ZiplineeReleaseAction{
									{
										Name:      "deploy-canary",
										HideBadge: false,
									},
						...
						ManifestObject: &manifest.ZiplineeManifest{},
					},
				}
				return
			})
		...
		buildService.
			...
			DoAndReturn(func(ctx context.Context, release contracts.Release, mft manifest.ZiplineeManifest, repoBranch, repoRevision string) (r *contracts.Release, err error) {
		...
		c.Set("JWT_PAYLOAD", jwt.MapClaims{
			...
			"email":         "user@ziplinee.io",
		})
		c.Params = append(c.Params, gin.Param{Key: "source", Value: "github.com"},
			gin.Param{Key: "owner", Value: "ziplineeci"},
			gin.Param{Key: "repo", Value: "ziplinee-ci-api"})
		bodyReader := strings.NewReader(`{  "name": "tooling-ziplinee",  "action": "deploy-canary",  "repoSource": "github.com",  "repoOwner": "ziplineeci",  "repoName": "ziplinee-ci-api",  "releaseVersion": "1.0.0-feature-test-123-1727",  "releaseStatus": "pending"}`)
		c.Request = httptest.NewRequest("POST", "https://ci.ziplinee.io/manifest/templates", bodyReader)
		if !assert.NotNil(t, c.Request) {
			return
		}
		// act
