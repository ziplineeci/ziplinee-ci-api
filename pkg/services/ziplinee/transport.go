...
func (h *Handler) CreatePipelineBuild(c *gin.Context) {
	...
	failedBuild.Events = []manifest.ZiplineeEvent{
		{
			Fired: true,
...
func (h *Handler) GenerateManifest(c *gin.Context) {
	...
	tmpl, err := template.New(".ziplinee.yaml").Parse(string(data))
	if err != nil {
...
func (h *Handler) Commands(c *gin.Context) {
	...
	log.Debug().Msgf("X-Ziplinee-Event-Job-Name is set to %v", c.GetHeader("X-Ziplinee-Event-Job-Name"))

	body, err := io.ReadAll(c.Request.Body)
	if err != nil {
		log.Error().Err(err).Msg("Reading body from Ziplinee 'build finished' event failed")
		c.String(http.StatusInternalServerError, "Reading body from Ziplinee 'build finished' event failed")
		return
	}

	// unmarshal json body
	var ciBuilderEvent contracts.ZiplineeCiBuilderEvent
	...
	case contracts.BuildEventTypeClean:
		...
		if ciBuilderEvent.GetStatus() != contracts.StatusCanceled {
			go func(eventJobname string) {
				...
				span, _ := opentracing.StartSpanFromContext(c.Request.Context(), "ziplinee:AsyncRemoveCiBuilderJob")
		...
		go func(ciBuilderEvent contracts.ZiplineeCiBuilderEvent) {
			// create new context to avoid cancellation impacting execution
			span, _ := opentracing.StartSpanFromContext(c.Request.Context(), "ziplinee:AsyncUpdateJobResources")
			ctx := opentracing.ContextWithSpan(context.Background(), span)
			defer span.Finish()
	...
	default:
		log.Warn().Msgf("Unsupported Ziplinee build event type '%v'", ciBuilderEvent.BuildEventType)
	}
