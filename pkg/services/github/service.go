...
type Service interface {
	...
	PublishGithubEvent(ctx context.Context, event manifest.ZiplineeGithubEvent) (err error)
...
func NewService(config *api.APIConfig, githubapiClient githubapi.Client, pubsubapiClient pubsubapi.Client, ziplineeService ziplinee.Service, queueService queue.Service) Service {
	return &service{
...
func (s *service) CreateJobForGithubPush(ctx context.Context, pushEvent githubapi.PushEvent) (err error) {
	...
	// get manifest file
	manifestExists, manifestString, err := s.githubapiClient.GetZiplineeManifest(ctx, accessToken, pushEvent)
	if err != nil {
		log.Error().Err(err).
...
func (s *service) PublishGithubEvent(ctx context.Context, event manifest.ZiplineeGithubEvent) (err error) {
