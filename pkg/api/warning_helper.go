...
type WarningHelper interface {
	GetManifestWarnings(*manifest.ZiplineeManifest, string) ([]contracts.Warning, error)
...
// NewWarningHelper returns a new ziplinee.WarningHelper
...
func (w *warningHelperImpl) GetManifestWarnings(manifest *manifest.ZiplineeManifest, fullRepoPath string) (warnings []contracts.Warning, err error) {
	warnings = []contracts.Warning{}

	isZiplineePipeline := strings.HasPrefix(fullRepoPath, "github.com/ziplineeci/")
	...
	if manifest != nil {
		...
		if len(stagesUsingLatestTag) > 0 {
			warnings = append(warnings, contracts.Warning{
				Status:  "warning",
				Message: fmt.Sprintf("This pipeline has one or more stages that use **latest** or no tag for its container image: `%v`; it is [best practice](https://ziplinee.io/usage/best-practices/#pin-image-versions) to pin stage images to specific versions so you don't spend hours tracking down build failures because the used image has changed.", strings.Join(stagesUsingLatestTag, ", ")),
			})
		}
		if len(stagesUsingDevTag) > 0 {
			warnings = append(warnings, contracts.Warning{
				Status:  "warning",
				Message: fmt.Sprintf("This pipeline has one or more stages that use the **dev** tag for its container image: `%v`; it is [best practice](https://ziplinee.io/usage/best-practices/#avoid-using-ziplinee-s-dev-or-beta-tags) to avoid the dev tag alltogether, since it can be broken at any time.", strings.Join(stagesUsingDevTag, ", ")),
			})
		}

		if manifest.Builder.Track == "dev" && !isZiplineePipeline {
			warnings = append(warnings, contracts.Warning{
				Status:  "warning",
				Message: "This pipeline uses the **dev** track for the builder; it is [best practice](https://ziplinee.io/usage/best-practices/#avoid-using-ziplinee-s-builder-dev-track) to avoid the dev track, since it can be broken at any time.",
			})
		}
		...
		for _, sv := range secretValues {
			...
			if pipelineAllowList == crypt.DefaultPipelineAllowList {
				warnings = append(warnings, contracts.Warning{
					Status:  "warning",
					Message: "This pipeline uses _global_ secrets which can be used by any pipeline; it is [best practice](https://ziplinee.io/usage/best-practices/#use-pipeline-restricted-secrets-instead-of-global-secrets) to use _restricted_ secrets instead, that can only be used by this pipeline. Please rotate the value stored in the secret and create a new one in the pipeline's secrets tab.",
				})
				break
			}
		}
