...
func TestReadConfigFromFiles(t *testing.T) {
	...
	t.Run("MergesMultipleYamlConfigFiles", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, "https://ci.ziplinee.io/", config.APIServer.BaseURL)
	...
	t.Run("ReturnsCloudsourceConfig", func(t *testing.T) {
		...
		assert.Equal(t, "ziplinee", cloudsourceConfig.ProjectOrganizations[0].Project)
		assert.Equal(t, 1, len(cloudsourceConfig.ProjectOrganizations[0].Organizations))
		assert.Equal(t, "Ziplinee", cloudsourceConfig.ProjectOrganizations[0].Organizations[0].Name)
	})
	...
	t.Run("ReturnsPubsubConfig", func(t *testing.T) {
		...
		assert.Equal(t, "ziplinee", pubsubConfig.DefaultProject)
		assert.Equal(t, "https://ci-integrations.ziplinee.io/api/integrations/pubsub/events", pubsubConfig.Endpoint)
		assert.Equal(t, "ziplinee-audience", pubsubConfig.Audience)
		assert.Equal(t, "ziplinee@ziplinee.iam.gserviceaccount.com", pubsubConfig.ServiceAccountEmail)
		assert.Equal(t, "~ziplinee-ci-pubsub-trigger", pubsubConfig.SubscriptionNameSuffix)
	...
	t.Run("ReturnsAPIServerConfig", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, "https://ci.ziplinee.io/", apiServerConfig.BaseURL)
		assert.Equal(t, "https://ci-integrations.ziplinee.io/", apiServerConfig.IntegrationsURL)
		assert.Equal(t, "http://ziplinee-ci-api.ziplinee.svc.cluster.local/", apiServerConfig.ServiceURL)
		...
		assert.Equal(t, "ziplinee", apiServerConfig.DockerConfigPerOperatingSystem[manifest.OperatingSystemLinux].Networks[0].Name)
	...
	t.Run("ReturnsAuthConfig", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, "ci.ziplinee.io", authConfig.JWT.Domain)
		...
		assert.Equal(t, ".+@ziplinee\\.io", authConfig.Google.AllowedIdentitiesRegex)
		...
		assert.Equal(t, ".+@ziplinee\\.io", authConfig.Github.AllowedIdentitiesRegex)
		...
		assert.Equal(t, ".+@ziplinee\\.io", authConfig.Organizations[0].OAuthProviders[0].AllowedIdentitiesRegex)
	...
	t.Run("ReturnsJobsConfig", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, "ziplinee-ci-jobs", jobsConfig.Namespace)
		assert.Equal(t, "ziplinee-ci-builder", jobsConfig.ServiceAccountName)
	...
	t.Run("ReturnsDatabaseConfig", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, "ziplinee_ci_api", databaseConfig.DatabaseName)
		assert.Equal(t, "cockroachdb-public.ziplinee.svc.cluster.local", databaseConfig.Host)
	...
	t.Run("ReturnsQueueConfig", func(t *testing.T) {
		...
		assert.Equal(t, "ziplinee-ci-queue-0.ziplinee-ci-queue", queueConfig.Hosts[0])
	...
	t.Run("AllowsCredentialConfigWithComplexAdditionalPropertiesToBeJSONMarshalled", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, "{\"name\":\"gke-ziplinee-production\",\"type\":\"kubernetes-engine\",\"additionalProperties\":{\"cluster\":\"production-europe-west2\",\"defaults\":{\"autoscale\":{\"min\":2},\"container\":{\"repository\":\"ziplinee\"},\"namespace\":\"ziplinee\",\"sidecars\":[{\"image\":\"ziplinee/openresty-sidecar:1.13.6.1-alpine\",\"type\":\"openresty\"}]},\"project\":\"ziplinee-production\",\"region\":\"europe-west2\",\"serviceAccountKeyfile\":\"{}\"}}", string(bytes))
	})
