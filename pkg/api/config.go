...
type APIConfig struct {
	...
	ManifestPreferences       *manifest.ZiplineeManifestPreferences `yaml:"manifestPreferences,omitempty"`
...
func (c *APIConfig) SetDefaults() {
	...
	if c.ManifestPreferences == nil {
		c.ManifestPreferences = &manifest.ZiplineeManifestPreferences{}
	}
...
func (c *APIServerConfig) SetDefaults() {
	if c.ServiceURL == "" {
		c.ServiceURL = "http://ziplinee-ci-api.ziplinee-ci.svc.cluster.local"
	}
...
func (c *APIServerConfig) Validate() (err error) {
	...
	if c.ServiceURL == "" {
		return errors.New("Configuration item 'apiServer.serviceURL' is required; please set it to a full http url towards the ziplinee api, for build/release jobs to report status and send logs to")
	}
	if len(c.LogWriters) == 0 {
...
type InjectStagesTypeConfig struct {
	Before []*manifest.ZiplineeStage `yaml:"before,omitempty"`
	After  []*manifest.ZiplineeStage `yaml:"after,omitempty"`
}
...
func (c *JobsConfig) SetDefaults() {
	...
	if c.ServiceAccountName == "" {
		c.ServiceAccountName = "ziplinee-ci-builder"
	}
...
func (c *QueueConfig) SetDefaults() {
	if len(c.Hosts) == 0 {
		c.Hosts = []string{"ziplinee-ci-queue-0.ziplinee-ci-queue"}
	}
	if c.SubjectCron == "" {
...
func (c *PubsubConfig) SetDefaults() {
	...
	if c.SubscriptionNameSuffix == "" {
		c.SubscriptionNameSuffix = "~ziplinee-ci-pubsub-trigger"
	}
...
func (c *PubsubConfig) Validate() (err error) {
	...
	if c.Endpoint == "" {
		return errors.New("Configuration item 'integrations.pubsub.endpoint' is required; please set it to the public http endpoint of Ziplinee CI suffixed by path /api/integrations/pubsub/events")
	}
	if c.Audience == "" {
	...
	if c.ServiceAccountEmail == "" {
		return errors.New("Configuration item 'integrations.pubsub.serviceAccountEmail' is required; please set it to the service account id used by Ziplinee CI to create Pub/Sub triggers")
	}
	if c.SubscriptionNameSuffix == "" {
...
func (c *PrometheusConfig) SetDefaults() {
	...
	if c.ServerURL == "" {
		c.ServerURL = "http://ziplinee-ci-metrics-server"
	}
