...
func isValidGoogleJWT(tokenString string) (valid bool, err error) {
	...
	if claims, ok := token.Claims.(jwtgo.MapClaims); ok && token.Valid {
		...
		// "aud": "https://ci.ziplinee/io/api/integrations/pubsub/events",
		// "azp": "118094230988819892802",
		// "email": "ziplinee@ziplinee.iam.gserviceaccount.com",
		// "email_verified": true,
		// "exp": 1568134507,
		// "iat": 1568130907,
...
func GetFilters(c *gin.Context) map[FilterType][]string {
	// get filters (?filter[status]=running,succeeded&filter[since]=1w&filter[labels]=team%3Dziplinee-team)
