...
type Client interface {
	...
	GetZiplineeManifest(ctx context.Context, accesstoken AccessToken, notification PubSubNotification, gitClone func(string, string, string) error) (valid bool, manifest string, err error)
...
func (c *client) GetZiplineeManifest(ctx context.Context, accesstoken AccessToken, notification PubSubNotification, gitClone func(string, string, string) error) (exists bool, manifest string, err error) {
	...
	gitUrl := fmt.Sprintf("https://ziplinee:%v@%v/p/%v/r/%v", accesstoken.AccessToken, repoSource, repoOwner, repoName)
	if gitClone == nil {
	...
	ziplineeFilename := filepath.Join(dir, ".ziplinee.yaml")
	if _, fileErr := os.Stat(ziplineeFilename); os.IsNotExist(fileErr) {
		exists = false
		manifest = ""
	...
	ziplineeFile, err := os.ReadFile(ziplineeFilename)
	if err != nil {
		return
	}

	exists = true
	manifest = string(ziplineeFile)
