...
func TestIntegrationGetAutoIncrement(t *testing.T) {
	t.Run("ReturnsAnIncrementingCountForUniqueRepo", func(t *testing.T) {
		...
		// act
		autoincrement, err := databaseClient.GetAutoIncrement(ctx, "github", "ziplineeci", "ziplinee-ci-api")
	...
	t.Run("ReturnsLargerCountForSubsequentRequests", func(t *testing.T) {
		...
		// act
		autoincrement1, err1 := databaseClient.GetAutoIncrement(ctx, "github", "ziplineeci", "ziplinee-ci-api")
		autoincrement2, err2 := databaseClient.GetAutoIncrement(ctx, "github", "ziplineeci", "ziplinee-ci-api")
...
func TestIngrationGetPipelines(t *testing.T) {
	...
	t.Run("ReturnsPipelinesForLabelsFilter", func(t *testing.T) {
		...
		filters := map[api.FilterType][]string{
			api.FilterLabels: {"app-group=ziplinee-ci"},
		}
	...
	t.Run("ReturnsPipelinesForRecentCommitterFilter", func(t *testing.T) {
		...
		filters := map[api.FilterType][]string{
			api.FilterRecentCommitter: {"me@ziplinee.io"},
		}
	...
	t.Run("ReturnsPipelinesForRecentReleaserFilter", func(t *testing.T) {
		...
		release := getRelease()
		release.Events = []manifest.ZiplineeEvent{
			{
				Fired: true,
				Manual: &manifest.ZiplineeManualEvent{
					UserID: "me@ziplinee.io",
				},
			},
		}
		...
		filters := map[api.FilterType][]string{
			api.FilterRecentReleaser: {"me@ziplinee.io"},
		}
...
func TestIntegrationGetUserByID(t *testing.T) {
	...
	t.Run("ReturnsInsertedUserWithIDFilteredOnOrganization", func(t *testing.T) {
		...
		assert.Nil(t, err)
		filters := map[api.FilterType][]string{
			api.FilterOrganizations: []string{"Ziplinee"},
		}
...
func TestIntegrationGetCatalogEntities(t *testing.T) {
	...
	t.Run("ReturnsInsertedCatalogEntitiesByLinkedPipeline", func(t *testing.T) {
		...
		databaseClient := getDatabaseClient(ctx, t)
		catalogEntity := getCatalogEntity()
		catalogEntity.LinkedPipeline = "github.com/ziplineeci/ziplinee-ci-api"
		_, err := databaseClient.InsertCatalogEntity(ctx, catalogEntity)
		assert.Nil(t, err)

		filters := map[api.FilterType][]string{
			api.FilterPipeline: {
				"github.com/ziplineeci/ziplinee-ci-api",
			},
		}
...
func TestIntegrationGetGitTriggers(t *testing.T) {
	t.Run("ReturnsPipelineIfTriggerEventAndRepositoryMatch", func(t *testing.T) {
		...
		build.Triggers = []manifest.ZiplineeTrigger{
			{
				Git: &manifest.ZiplineeGitTrigger{
					Event:      "push",
...
func TestIntegrationGetGithubTriggers(t *testing.T) {
	t.Run("ReturnsPipelineIfOneTriggerEventMatches", func(t *testing.T) {
		...
		build.Triggers = []manifest.ZiplineeTrigger{
			{
				Github: &manifest.ZiplineeGithubTrigger{
					Events: []string{
						"commit_comment",
						"create",
						"delete",
		...
		githubEvent := manifest.ZiplineeGithubEvent{
...
func TestIntegrationGetBitbucketTriggers(t *testing.T) {
	t.Run("ReturnsPipelineIfOneTriggerEventMatches", func(t *testing.T) {
		...
		build.Triggers = []manifest.ZiplineeTrigger{
			{
				Bitbucket: &manifest.ZiplineeBitbucketTrigger{
					Events: []string{
						"pullrequest:created",
						"pullrequest:updated",
						"pullrequest:approved",
					},
					Repository: "bitbucket.org/ziplineeci/ziplinee-ci-api",
				},
			},
		}
		...
		bitbucketEvent := manifest.ZiplineeBitbucketEvent{
...
func getBuild() contracts.Build {
	return contracts.Build{
		...
		Labels:         []contracts.Label{{Key: "app-group", Value: "ziplinee-ci"}, {Key: "language", Value: "golang"}},
		...
		Commits: []contracts.GitCommit{
			{
				...
				Author: contracts.GitAuthor{
					Email: "me@ziplinee.io",
				},
			},
		},
		Triggers: []manifest.ZiplineeTrigger{},
...
func getNotification() contracts.NotificationRecord {
	return contracts.NotificationRecord{
		...
		LinkID:   "github.com/ziplineeci/ziplinee-ci-api",
...
func getClient() contracts.Client {
	...
	return contracts.Client{
		Name:         "ziplinee-cron-event-sender",
		ClientID:     "abc",
