...
type Client interface {
	...
	GetZiplineeManifest(ctx context.Context, accesstoken AccessToken, event RepositoryPushEvent) (valid bool, manifest string, err error)
...
func (c *client) GetZiplineeManifest(ctx context.Context, accesstoken AccessToken, pushEvent RepositoryPushEvent) (exists bool, manifest string, err error) {
	...
	manifestSourceAPIUrl := fmt.Sprintf("https://api.bitbucket.org/2.0/repositories/%v/src/%v/.ziplinee.yaml", pushEvent.Repository.FullName, pushEvent.Push.Changes[0].New.Target.Hash)
	...
	if response.StatusCode != http.StatusOK {
		err = fmt.Errorf("Retrieving ziplinee manifest from %v failed with status code %v", manifestSourceAPIUrl, response.StatusCode)
		return
	}
...
const bitbucketConfigmapName = "ziplinee-ci-api.bitbucket"
