...
func (c *client) GetMaxMemoryByPodName(ctx context.Context, podName string) (maxMemory float64, err error) {
	...
	query := fmt.Sprintf("max_over_time(container_memory_working_set_bytes{container=\"ziplinee-ci-builder\",pod=\"%v\"}[6h])", podName)
...
func (c *client) GetMaxCPUByPodName(ctx context.Context, podName string) (maxCPU float64, err error) {
	...
	query := fmt.Sprintf("max_over_time(container_cpu_usage_rate1m{container=\"ziplinee-ci-builder\",pod=\"%v\"}[6h])", podName)
